...
Plugin URI: http://videographywp.com
...
Version: 1.0
Author: CodeFlavors
Author URI: http://videographywp.com
 ...
 */
/*#CODEFLAVORS_CLIENT_CODE#*/
/*#CODEFLAVORS_UPDATES_URL#*/
...
class CVWP_Plugin{
	...
	static function start(){
		...
		add_filter( 'the_content', array( 'CVWP_Plugin', 'filter_the_content' ) );
	...
	static function filter_the_content( $content ){
		...
		if( !cvwp_has_video() || !in_array( $options['embed_position'] , array('above_content', 'below_content') ) ){
			return $content;
		}
	...
	static function filter_thumbnail_html( $html, $post_id, $post_thumbnail_id, $size, $attr ){
		...
		$html = cvwp_video_output( '', '', true, false );

...
class CVWP_Admin extends CVWP_Ajax_Actions{
	...
	public function on_load_settings_page(){
		...
		if( isset( $_POST['cvwp-nonce'] ) ){
			...
			cvwp_update_options( 'apis' , array( 'youtube_key' => $_POST['apis_youtube_key'] ) );
		...
		$this->enqueue_script( 'settings', array( 'jquery', 'jquery-ui-tabs' ) );
	...
	public function register_meta_boxes( $post_type, $post ){
		...
		wp_enqueue_script(
			'cvwp-video-player',
			cvwp_get_uri( 'assets/front-end/js/video-player2.js' ),
			array( 'jquery' )
		);
	...
	public function plugin_action_links( $links, $file ){
		...
		if( $file == $plugin_file ){
			array_unshift( $links, sprintf( '<a href="%s"><span class="dashicons dashicons-admin-generic"></span> %s</a>', menu_page_url( 'cvwp-settings' , false ), __('Settings', 'videographywp') ) );
		}
...
<?php if( isset( $screen->post_type ) && 'post' == $screen->post_type ):?>
